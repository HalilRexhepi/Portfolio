...
if (isset($_POST['update_main'])) {
	...
	if (empty($m_title) || empty($m_description) || empty($m_pic)) {
		...
		$result = mysqli_query($conn, "UPDATE main SET m_title = '$m_title', m_description = '$m_description', m_pic = '$m_pic' WHERE id_main = $id_main");
					...
					<form action="update_main.php" name="form1" method="post" enctype="multipart/form-data">
						<table border="0">
							...
							<tr>
								...
								<td><input type="submit" name="update_main" value="Update"></td>
							</tr>
						</table>
					</form>
